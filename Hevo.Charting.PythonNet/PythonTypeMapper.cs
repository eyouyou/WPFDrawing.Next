...
namespace Hevo.Charting.PythonNet
{
    ...
    public static class PythonTypeMapper
    {
        ...
        // §D2.6.4: 返回类型支持 "Tuple[X, Y]" 嵌套形态,X / Y 自身可含 ReadOnlyMemory[double]。
        // 故 ret 段允许 [, ] 和逗号(此前只允许 [\w\[\]\.] 不带逗号)。
        private static readonly Regex _signatureRegex = new(
            @"^\s*\(\s*(?<params>.*?)\s*\)\s*(->\s*(?<ret>[\w\[\]\.,\s]+?))?\s*$",
        ...
        public static Type? ResolveDelegateType(string signature)
        {
            ...
            // 参数列表 (空字符串 = 0 参数)
            // §D2.6.4: 走 SplitTopLevelCommas 而非 string.Split(',') —— 嵌套 Tuple[...] 参数也能切对。
            var paramTypes = string.IsNullOrEmpty(paramsRaw)
                ? Array.Empty<Type>()
                : SplitTopLevelCommas(paramsRaw).Select(MapType).ToArray();
        ...
        /// 一层泛型 <c>ReadOnlyMemory[T]</c> / <c>Tuple[T1, T2, ...]</c>(§D2.6.4 增量协议返回值)。
        /// </summary>
        public static Type? MapType(string name)
        {
            ...
            if (lb > 0 && name.EndsWith("]"))
            {
                ...
                // §D2.6.4: Tuple[T1, T2, ...] / tuple[T1, T2, ...] → ValueTuple<T1, T2, ...>
                if (string.Equals(head, "Tuple", StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(head, "tuple", StringComparison.Ordinal))
                {
                    var args = SplitTopLevelCommas(inner);
                    if (args.Count < 2 || args.Count > 7) return null;  // ValueTuple<...> 1 ~ 7 元
                    var typeArgs = new Type[args.Count];
                    for (int i = 0; i < args.Count; i++)
                    {
                        var t = MapType(args[i]);
        ...
        /// <summary>
        /// 顶层逗号切分(尊重嵌套 <c>[]</c>)。例 "ROM[double], ROM[double]" → ["ROM[double]", "ROM[double]"]。
        /// 用于 Tuple[...] / 多元素参数列表的递归 type 解析。
        /// </summary>
        internal static List<string> SplitTopLevelCommas(string s)
        {
            ...
            else if (start == s.Length && s.Length > 0 && s[s.Length - 1] == ',')
                result.Add("");
            return result;
        }
    }
